...
int main() {
	...
	sf::RenderWindow window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT), "Maze Visualizer");

	int selectedAlgo = 0;
	runMenu(window, selectedAlgo); // Show the menu and set algorithm choice

	// Clear screen before algorithm runs
	window.clear(sf::Color::Black);
	window.display();
	sf::sleep(sf::seconds(1)); // Optional pause

	// Run selected algorithm or comparison
	if (selectedAlgo >= 1 && selectedAlgo <= 5) {
		runAlgorithm(selectedAlgo); // e.g. BFS, DFS, Dijkstra, A*
	}
	else if (selectedAlgo == 6) {
		compareAlgorithms(); // if you have that implemented
	}

	// Show the maze visually until window is closed
	while (window.isOpen()) {
		sf::Event event;
		while (window.pollEvent(event)) {
			if (event.type == sf::Event::Closed)
		...
		window.clear();
		drawGrid(window); // Draw the maze and path visually
		window.display();
	}
